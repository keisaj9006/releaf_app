...
**Open gap:** individual steps have no `advanceActionLabel`, so explicit labelled
skip/ready controls still need implementation and accessibility verification.
...
- Next milestone: explicit accessible skip/ready controls; verify silent/no-words,
  reduced-motion, large-text, pause and path changes with those controls.
