...
| Sleep information architecture | Existing screen manually groups bundled Sound IDs and older documents describe all Sleep as narration-free | Stories / Nature / Meditations / Sleep Music with `All` as an aggregate | **Owner resolved, 23 September 2026:** the four-category structure is frozen. Nature and Sleep Music remain narration-free; Stories and guided Sleep Meditations may use approved narration. Register missing production assets honestly and preserve the legacy Sound library/routes. |
| Sleep Stories player | No Story model/player exists; Sound loops and Meditation owns separate voice/ambience playback | One scalable long-form Story library and reliable shared playback foundation | Extend the existing Sound/AudioService boundary for finite media and persistent progress before adding Story UI. Do not create a second background audio service or 50 bespoke screens. |
