...
| Release AAB | CI SMOKE DONE / PROD SIGNING REQUIRED | `ecd3e97` builds/uploads the release AAB smoke artifact and passes the 16 KB compatibility gate in run `34834895182`. The smoke AAB GitHub artifact is `10343214232` with artifact-ZIP SHA-256 `da6b90145ade640edd78d82a640d159abdbf6dde1f24667f80eef31e803dcb87`. The local and GitHub production paths now fail closed above the measured 105,000,000-byte AAB and 63,000,000-byte release source-asset-tree budgets; see [size-budget evidence](2026-09-24-release-size-budgets.md). The smoke artifact uses the short-lived CI signing key and is not the Play upload artifact. Final Play artifact still requires the production signing workflow and private upload key. |
...
| Store listing | COPY + LEGAL CONTACTS + APP ICON READY / REMAINING ASSETS + PLAY ENTRY REQUIRED | Canonical UK-English copy markets only the active Reset / Brain / Sleep 1.0 surface, with Emergency Calm and local-first progress described accurately. Support/privacy email `canius.uk@gmail.com`, public Privacy URL and public Account Deletion URL are known. The 512 × 512 RGBA Play icon is a measured derivative of the existing Releaf launcher mark with pinned source/output hashes; see [icon evidence](2026-09-24-play-store-icon.md). `tool/release/play_store_asset_policy.dart` is contract-tested, and `tool/build_play_release.ps1` enforces the `--strong-listing` asset gate before a production Play build can continue. Final closure requires current RC screenshots, the feature graphic, optional/stable website if used, owner review and Play Console entry/preview. |
