...
| Sleep player/timer | IN PROGRESS / CONTENT + PLAYER + QA | Existing looping Sound playback/timer behaviour remains tested, including delayed-start cancellation, media-notification intent and volume restoration after interrupted expiry. On 23 September the owner froze a broader Sleep structure: Stories / Nature / Meditations / Sleep Music. The canonical registry foundation is now in progress; long-form Story persistence, finite playback, discovery UI, approved production assets and physical background/interruption QA remain open. Nature and Sleep Music stay narration-free; Stories and guided Sleep Meditations may use approved narration. |
...
| Release AAB | CI SMOKE DONE / PROD SIGNING REQUIRED | `ecd3e97` builds/uploads the release AAB smoke artifact and passes the 16 KB compatibility gate in run `34834895182`. The smoke AAB GitHub artifact is `10343214232` with artifact-ZIP SHA-256 `da6b90145ade640edd78d82a640d159abdbf6dde1f24667f80eef31e803dcb87`. This artifact uses the short-lived CI signing key and is not the Play upload artifact. Final Play artifact still requires the production signing workflow and private upload key. |
