...
2. A valid Android artifact must have been uploaded to an appropriate Play track so subscription products can be configured/tested.
...
2. Connect the Google Play store credentials required by RevenueCat.
3. Import the two active Google base-plan products.
4. Create or verify entitlement exactly named `premium`.
5. Attach both monthly and annual Google products to `premium`.
6. Create or reuse one Offering intended for Releaf 1.0 and mark it **current**.
7. Add the annual product as the standard **Annual** package type.
8. Add the monthly product as the standard **Monthly** package type.
9. Do not use Custom package types for these two 1.0 subscriptions; the app reads `current.annual` and `current.monthly`.
10. Copy the Android **public SDK key** beginning with `goog_`; never use a RevenueCat secret key in the app or GitHub build.
...
No keystore, password, `key.properties`, RevenueCat secret key, or service-role credential may be committed to the repository.
...
- Monthly purchase succeeds with a licensed/test account.
- Annual purchase succeeds or is separately verified with an appropriate controlled test scenario.
...
- No real financial purchase is made outside an authorized Google Play test context.
...
- RevenueCat Google Play credentials connection
...
- Play-distributed purchase + restore verification

Until those items are evidenced, the canonical release gate remains `CODE READY / EXTERNAL CONFIG REQUIRED` for RevenueCat / Google Play Billing.
...
- RevenueCat: Configuring Products — products attach to entitlements; Offerings group packages; the default/current Offering is returned by the SDK.
